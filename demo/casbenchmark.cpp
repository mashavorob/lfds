...
struct Runner
{
	...
	void operator()(std::size_t repetitions, const Value tag, const Value expected, const Value newVal)
	{
		do
		{
			Value tagVal = tag;
...
struct RunnerTraits
{
	...
	static void run(std::size_t repetitions, const Value tag, const Value expected, const Value newVal)
	{
...
struct Benchmark {
	...
	{
		std::cout
			<< "Benchmark for " << runner_traits_type::name() << std::endl
			<< "---------------------------------" << std::endl;
		runtest(tag, exp, val);
		runtest(tag, tag, val);
		std::cout
...
template<class Value>
void doBenchmark(const Value tag, const Value exp, const Value val, bool builtin)
{
	if ( builtin )
	{
