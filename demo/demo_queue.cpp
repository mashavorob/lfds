...
public:
    static void demonstrate()
    {
        ...
        std::cout << prefix[fixed_collection_type::wait_free]
                << " fixed size queue:" << std::endl;
        double fixed_perf = benchmark(fixed_coll, duration, num_producers,
                num_consumers);
        std::cout << "size at exit: " << fixed_coll.size() << std::endl
                << std::endl;

        std::cout << prefix[dynamic_collection_type::wait_free]
                << " dynamic size queue:" << std::endl;
        ...
        std::cout << "Reference performance: "
                << static_cast<int>(1e9 / ref_perf) << " ns per item "
                << std::endl;
        std::cout << "     Fixed size queue: "
                << static_cast<int>(1e9 / fixed_perf) << " ns per item "
                << fixed_perf / ref_perf << " of reference performance"
                << std::endl;
        std::cout << "   Dynamic size queue: "
                << static_cast<int>(1e9 / dynamic_perf) << " ns per item "
                << dynamic_perf / ref_perf << " of reference performance "
                << std::endl;
        std::cout << std::endl;
            ...
            num_producers, const unsigned numconsumers = num_consumers)
    {
        ...
        { "Single Producer", "Many Producers" };
        static const char* cs[] =
        { "Single Consumer", "Many Consumers" };
        ...
        {
            std::cout << "Profiling "
                    << prefix[fixed_collection_type::wait_free] << " "
                    << pr[ManyProducers != false] << " "
                    << cs[ManyConsumers != false] << " "
                    << ss[fixedSize != false] << " queue" << std::endl;
            perf = benchmark(fixed_coll, duration, numproducers, numconsumers);
        }
        else
        {
            std::cout << "Profiling "
                    << prefix[dynamic_collection_type::wait_free] << " "
                    << pr[ManyProducers != false] << " "
                    << cs[ManyConsumers != false] << " "
                    << ss[fixedSize != false] << " queue" << std::endl;
        ...
        std::cout << "Finished: " << static_cast<int>(1e9 / perf)
